class Section < ApplicationRecord
  ...
  after_validation do |section|
    if section.title == "前回の開催の様子"
      ...
      section.text += ("<div id='last_lesson'><a class='image' #{a_href}><img src='#{ this_blog.image.attachment.service.send(:object_for, this_blog.image.key).public_url }'></a><a class='title' #{a_href}>#{ last_blog.title }</a></div>")
    end
  end
end
