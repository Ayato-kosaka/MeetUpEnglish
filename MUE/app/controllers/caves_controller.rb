class CavesController < ApplicationController
  ...
  def update
    ...
    unless params[:cafe][:image_ids].nil?
      params[:cafe][:image_ids].each do |image_id|
        image = @cafe.images.find(image_id)
        image.purge
      end
    end
  end
  ...
  private
    ...
    def cafe_params
      params.require(:cafe).permit(:name, :description, :city_id, :url, images: [])
    end
end
